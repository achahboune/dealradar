...
export default function FlashRadarPage() {
  ...
  // Charger les deals "flash"
  useEffect(() => {
    const fetchDeals = async () => {
      try {
        const res = await fetch("/api/deals/radar?flashOnly=true");
        const data = await res.json();
        setDeals(data.deals || []);
      } catch (e) {
        console.error(e);
  ...
  const handleRank = async () => {
    ...
    try {
      const filtered = deals.filter((d) => d.currentPrice <= maxPrice);
      const res = await fetch("/api/gemini/rank-deals", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ deals: filtered, preferences: { maxPrice, categories: [], favoriteStores: [] } }),
      });
  ...
  const dealsMap = Object.fromEntries(deals.map((d) => [d.id, d] as const));

  return (
    <main className="min-h-screen bg-black text-white flex flex-col items-center px-4 py-10">
      <div className="max-w-3xl w-full">
        <header className="mb-8 text-center">
          <h1 className="text-3xl md:text-5xl font-bold mb-3">⚡ DealRadar – Flash Radar (≤ 15 min)</h1>
          <p className="text-gray-300 mb-4">
            Only shows deals that are about to disappear. Perfect for last-minute Black Friday snipes.
          </p>
        </header>

        <section className="mb-6 bg-zinc-900 border border-zinc-800 rounded-2xl p-4 md:p-6">
          <h2 className="text-xl font-semibold mb-4">Flash deal filters</h2>
          <div className="flex flex-col md:flex-row gap-4 items-center">
            ...
            >
              {loading ? "Scanning flash deals..." : "Scan flash deals"}
            </button>
          </div>
          {loadingDeals && <p className="text-xs text-gray-400 mt-2">Loading flash deals...</p>}
        </section>

        <section className="grid md:grid-cols-2 gap-4">
          {/* LEFT */}
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
            <h3 className="text-lg font-semibold mb-3">Flash deals (≤ 15 min)</h3>
          ...
          {/* RIGHT */}
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
            <h3 className="text-lg font-semibold mb-3">AI-ranked flash deals</h3>
